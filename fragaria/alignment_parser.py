...
import re
import sys
from fragaria.assembly_parser import Assembly
...
class GAFRecord(object):
    def __init__(self, record_line_split, parse_cigar=False):
        ...
        self.path = parse_gaf_path(self.path_str)
        ...
        if parse_cigar and "cg" in self.optional_fields:
            self.cigar = split_cigar_str(self.optional_fields["cg"])
        else:
            self.cigar = None
...
class GraphAlignRecords(object):
    def __init__(self, gaf_file, parse_cigar=False, min_aligned_path_len=0, min_align_len=0, min_identity=0.,
                 trim_overlap_with_graph=False, assembly_graph=None, log_handler=None):
        self.gaf_file = gaf_file
        self.records = []
        self.parse_gaf(gaf_file, parse_cigar=parse_cigar, min_align_len=min_align_len,
                       min_aligned_path_len=min_aligned_path_len, min_identity=min_identity,
                       trim_overlap_with_graph=trim_overlap_with_graph, assembly_graph=assembly_graph,
                       log_handler=log_handler)

    def parse_gaf(self, gaf_file, parse_cigar=False, min_aligned_path_len=0, min_align_len=0, min_identity=0.,
                  trim_overlap_with_graph=False, assembly_graph=None, log_handler=None):
        with open(gaf_file) as input_f:
            for line_split in csv.reader(input_f, delimiter="\t"):
                self.records.append(GAFRecord(line_split, parse_cigar=parse_cigar))
        if min_aligned_path_len:
            go_r = 0
            while go_r < len(self.records):
                if self.records[go_r].p_align_len < min_aligned_path_len:
                    del self.records[go_r]
                else:
                    go_r += 1
        if min_align_len > min_aligned_path_len:
            go_r = 0
            while go_r < len(self.records):
                if self.records[go_r].align_len < min_align_len:
                    del self.records[go_r]
                else:
                    go_r += 1
        if min_identity:
            go_r = 0
            while go_r < len(self.records):
                if self.records[go_r].identity < min_identity:
                    del self.records[go_r]
                else:
                    go_r += 1
        if trim_overlap_with_graph:
            if isinstance(assembly_graph, Assembly) and assembly_graph.overlap():
                ...
                while go_r < len(self.records):
                    ...
                    if len(this_record.path) > 1:
                        ...
                        head_vertex_len = assembly_graph.vertex_info[this_record.path[0][0]].len
                        tail_vertex_len = assembly_graph.vertex_info[this_record.path[-1][0]].len
            ...
            sys.stderr.write("Assembly graph not available, overlaps untrimmed!\n")

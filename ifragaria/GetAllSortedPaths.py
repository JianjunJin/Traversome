...
from .utils import ProcessingGraphFailed, smart_trans_for_sort
...
class GetAllPaths(object):
    ...
    def run(self):
        ...
        for vertex_n in self.graph.vertex_info:
            if self.graph.vertex_info[vertex_n].seq[True] == self.graph.vertex_info[vertex_n].seq[False]:
                temp_f = self.graph.vertex_info[vertex_n].connections[True]
                temp_r = self.graph.vertex_info[vertex_n].conncetions[False]
        ...
        for go_set, v_set in enumerate(self.vertex_clusters):
            ...
            if is_closed:
                if 1 in self.copy_to_vertex[1] and bool(self.copy_to_vertex[1] & v_set):
                    single_copy_v = \
                    sorted(self.copy_to_vertex[1] & v_set, key=lambda x: -self.graph.vertex_info[x].len)[0]
        ...
        if not self.__paths:
            ...
            if mode == "embplant_pt":
                if len(sorted_paths) > 2 and \
                        not (100000 < sum(
                            [len(self.graph.export_path(part_p).seq) for part_p in sorted_paths[0][0]]) < 200000):
                    if log_handler:
    ...
    def __directed_graph_solver(
            self, ongoing_paths, next_connections, vertices_left, in_all_start_ve, undirected_vertices):
        if not vertices_left:
            new_paths, new_standardized = self.__standardize_paths(ongoing_paths, undirected_vertices)
        ...
        for next_vertex, next_end in next_connections:
            ...
            if next_vertex in vertices_left:
                ...
                if not new_left:
                    new_paths, new_standardized = self.__(new_paths, undirected_vertices)
                    if new_standardized not in self.__paths_set:
                        self.__paths.append(new_paths)
                        self.__paths_set.add(new_standardized)
                    return
        ...
        if not find_next:
            ...
            while new_all_start_ve:
                ...
                if new_start_vertex in vertices_left:
                    ...
                    if not new_left:
                        new_paths, new_standardized = self.__standardize_paths(new_paths, undirected_vertices)
                        if new_standardized not in self.__paths_set:
                            self.__paths.append(new_paths)
                            self.__paths_set.add(new_standardized)
                    else:
